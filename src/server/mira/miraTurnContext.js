const PRIOR_REFERENCE =
  /\b(?:it|that|those|them|each one|both|their capabilities|these (?:platforms|services|offerings|options)|the above (?:two|platforms|services|offerings|options)|the (?:first|second|third|fourth|last) one|the previous option|previous option|what about|tell me more|which (?:one|of those)|under that service|same (?:one|option|service|platform)|(?:explain|describe|tell me about|what does)\s+(?:(?:the|above|that)\s+)*(?:(?:first|second|third|fourth|last|[1-4](?:st|nd|rd|th))\s+(?:listed\s+)?(?:one|option|platform|service|offering|item)|(?:platform|service|option|item)\s+number\s+(?:one|two|three|four)|number\s+(?:one|two|three|four)))\b/i;
