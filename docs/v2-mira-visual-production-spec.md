...
Status update: all six static Mira portraits have been approved and wired for playback-adjacent visual presence under identity profile `mira-visual-v1`. This completes the static identity set for the current prototype.
...
The approved portrait assets are available as optimized WebP files under `public/images/agents/mira/`. The UI still keeps a branded local placeholder with `MV` initials and visible posture text as the image-error fallback.

The six portraits use the same person, natural square framing, lighting direction, wardrobe family, and background style.
